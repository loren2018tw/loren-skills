...
- One concept per card: Do not stuff an entire paragraph of definitions into a single card.
- Front is the "Question" to force active recall; Back is the "Answer," kept to one or two lines whenever possible.
- Coverage: Core definitions, key numbers/years, easily confused concept comparisons, and common exam topics.
- Comparison cards: For two concepts easily confused, make an additional "comparison card" (e.g., What is the difference between A and B?).
- Verbatim precision: Keep precise numbers, legal article numbers, and proper terms exactly as they appear in the study material—do not rewrite or infer.
- Flag uncertainties: If something is uncertain or not clearly explained in the material, append "(Please check original study material)" to the answer.
- "One answer, multiple questions" principle: A single core concept can be asked from different angles, with each question made into a separate card.
